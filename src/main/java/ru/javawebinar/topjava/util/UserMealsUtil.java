...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        HashMap<LocalDate, Integer> byDateCalories = new HashMap<>();
        meals.forEach(um -> byDateCalories.put(um.getDate(), um.getCalories() +
                (byDateCalories.getOrDefault(um.getDate(), 0))));
        List<UserMealWithExcess> result = new ArrayList<>();
        meals.forEach(um -> {
            if (TimeUtil.isBetweenHalfOpen(um.getTime(), startTime, endTime)) {
                result.add(
                        new UserMealWithExcess(um.getDateTime(), um.getDescription(),
                                um.getCalories(), byDateCalories.get(um.getDate()) > caloriesPerDay));
            }
        });
        return result;
    }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> byDateResult = meals.stream()
        ...
        return meals.stream()
                ...
                .map(um -> new UserMealWithExcess(um.getDateTime(), um.getDescription(), um.getCalories(),
                        byDateResult.get(um.getDate()) > caloriesPerDay)).collect(toList());
    }
}
